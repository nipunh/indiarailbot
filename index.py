...
def get_bot_response():
	...
	info = check_intent(userText,api_key)
	...
	if intent == 4:
		...
		info_next=check_intent(prev_msg,api_key)
		...
		if prev_intent == 1:
			if len(number) != 0:
				info_next["pnr_number"] = info["pnr_number"]
				# print(info_next["pnr_number"])
			ans = pnr_enqiury(info_next)
			info_next["html"] = ans
			info_next["debit"] = 1
			info_next["response_code"] = 200
			f = open("log.txt", "a")
			f.write("Query Executed Successfully"+"\n")
			f.close()
			return(info_next)	
		...
		if prev_intent == 3:
			...
			ans=train_bw_station(info_next)
			return(ans)
...
def Train_arrivals(info):
	station_code=info["station_code"]
	time = info["time_bracket"]
	print(station_code,time)
	if station_code == None or time == None:
		info["debit"] = 4
		info["response_code"] = 502
		return(info)
	else:
		try:
			train_route_url = "https://api.railwayapi.com/v2/arrivals/station/"+station_code+"/hours/"+str(time)+"/apikey/"+api_key+"/"
			response_ob = requests.get(train_route_url)
			result = response_ob.json()
			print(result)
			if result['response_code'] == 200:
...
def check_intent(user_text,api_key):
	# print("Hello")
...
#Intent Classification	
	...
	info={"debit":None,"response_code":None, "bot_reply":None, "station_code":None, "train_num":-1, "src_stn":None, "dest_stn":None, "words":None,
		"api_key":api_key, "second_msg":None, "date":None, "time_bracket":None, "pnr_number":-1, "html":None}
	#route_inent=0 ,pnr_intent=1 , liveStatus_intent=2
	...
	['train','between','from','stations','station','trains','to','destination'],['train','after','from','station','hour', 'hours','next','within']]
	
	intent_score=[]
	intent_dict={ 0: "route_inent",1:"pnr_intent",2:"liveStatus_intent",3:"Train_bw_station",4:"Train_arrivals"}
	info["date"]=date
	
	res=[]
	for i in range(0,len(data)):
		res.append([val for key, val in station_code_name.items() if data[i] in key])
...
'and', 'but', 'if', 'or', 'because', 'as', 'until', 'while', 'of', 'at', 'by', 'for', 'with', 'about', 'against', 'into', 'through', 'above',
